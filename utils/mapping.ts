...
import { addPropToTarget } from './constructTarget';
import { querySingleProp, queryAll } from './queryJson';
import { validateWithSchema, validationUtil } from '../schema/validator';
import { pickTemplateVarsFromString, wrapInVarBraces } from './stringUtils';
import { formatPropValueIfNecessary } from './formattingUtils';
...
export const streamBatchProcess = (inputDir, outputDir, xFormTemplate) => {
  ...
  async.forEachOf(filesToTransform, (value) => {
...
const flattenEverything = (everything) => {
  ...
  for (const [key, val1] of Object.entries(everything)) {
    ...
    if (val.constructor === Array && val.length === 1) {
      ...
      // @ts-expect-error: This is just how it works, ask the original author
...
const addBlockToTarget = (block, target, flatten) => {
  ...
  if (flatten) {
    ...
    if (block.constructor === Array) newTarget.push(...block);
    else newTarget.push(block);
  }
  return newTarget;
};
